...
**94. A re-run control was added to the Squads screen, against three places that say there is no re-run button.** `app-spec.md` under "Keeping the squads current", `allocation-rules.md` under "Re-running the allocation", and the supersession recorded at item 22 all say the same thing: the allocation re-runs by itself whenever anything changes it, so there is nothing left for a button to do. The control went in anyway, and the contradiction was flagged rather than resolved.

That reasoning was sound when it was written, and it stopped being sound at item 54. Once a manual move is **pinned**, re-running is no longer the same operation as any change event. A change event re-runs around whatever pins exist as a side effect of something else happening; what an admin wants after moving four children by hand is to re-run *deliberately*, keeping those four where they were put and letting everyone else settle around them. Nothing triggers that. Clearing all pins does not either — that is the opposite action, and it throws the four moves away.

**Decided.** A re-run control exists on the Squads screen. It is **always enabled**, and where it moves nobody it says so, because "nothing moved" is the useful answer and an error message for a predictable outcome is worse — the same reasoning already recorded in the re-run paragraph of `allocation-rules.md` for the automatic case. **Clearing all pins stays** as the separate way to get a clean allocation from scratch.

This supersedes the no-button half of item 22. Everything else recorded there stands: the allocation still re-runs by itself on every change, a re-run is never silent, pinned people stay put, and re-publishing notifies only those whose squad changed. Both spec files are updated.

**Recorded rather than fixed:** a re-run after a manual move currently reshuffles heavily. Moving one child and re-running moved 22 children in testing, where `allocation-rules.md` asks for "the fewest moves that satisfy the rules, so a late change doesn't reshuffle the session". The prototype's allocator rebuilds from scratch around the pins rather than perturbing the existing arrangement, so every re-run is a fresh deal that happens to honour the pins. This pre-dates the control — every automatic re-run has always done it — and the control only makes it easier to notice. The minimal-move requirement is real and unbuilt.


**95. The admin and parent calendars had drifted apart in ways no decision covered.** They were built separately — the parent's first, the admin's later — and a line-by-line comparison of the two renderers turned up four differences that nothing in either spec had asked for. There is no earlier-events collapse on the admin side, so an Under 9 admin gets the whole season from August in one list. Past events recede on both, but differently: the admin's is a filled `--surface-2` panel, which is lighter than the page ground and therefore sits *forward* of it, and the parent's is transparent with a hairline. The admin has no date tile at all, only plain text where the parent has a colour by type. And the admin's expanded card shows the venue name and nothing else, where a parent gets the eircode, the map link and the access note.

**Decided.** All four align to the parent. That direction is not arbitrary: the parent's side is the one that has been designed, argued over and read against the colour rules, and it is the side the trial will put in front of people. Aligning the other way would mean re-deciding four things that were decided once already.

Each of the four was written down in the parent's section of `app-spec.md`, and in three of them the *reasoning* turns out not to be parent-specific at all. **A list opens where the action is** — that is why the earlier-events collapse exists, and it holds harder for an admin reading thirteen events in September than for a parent reading their child's four. **A coloured tile is faster to scan than plain text**, per item 75, and scanning is more of what an admin does with a calendar than a parent does. The **venue gap was simply the reverse of what item 86 intended**: that item says whether a venue's details are complete is an admin's problem and the admin's view is where it belongs, and the admin's view was the one place the details were absent.

So the collapse behaves as the parent's does, down to the turning chevron and the guard against printing a month heading twice; it is a collapse and not a filter, drafts stay visible because they are admin-only anyway, and a cancelled event stays inline wherever it falls. The past-event rule is now literally the same declaration, checked afterwards to match on every property. The tile is now the same component — the admin row carries `.ev-when` rather than a tile of its own — so the two cannot drift again. And the admin card carries the venue block, with one addition a parent never sees: where a venue has **no eircode recorded**, the admin is told so in as many words. That is a fact about the record rather than a pending state, and item 86's removal of "to be confirmed" from the parent's view is untouched.

One consequence worth naming, because it was not on the list: the split between "earlier" and the rest now uses the same predicate on both screens, `hasStarted()` rather than the admin's old whole-day `past` flag. Without that a session finishing at teatime would be earlier on one calendar and not on the other, and the two could not be said to match.

**Recorded as deliberately divergent, so it is not raised again as drift:**

- **The admin tile's second line is the weekday; the parent's is the month.** The admin row has no relative day in its subline — no "Today", no "Tomorrow" — so the weekday is the only place that information lives. The parent's subline carries it already, so the month is the more useful second line there.
- **The admin's collapsed row carries the full time range and the venue; the parent's carries the relative day and the meet time.** Both are specified and they are deliberately opposite: item 74 splits the times for a parent because the meet time is the instruction and the range is reference, and item 90 keeps the range and the venue on the admin row because the admin is reading a schedule rather than an instruction.
- **The admin keeps the response counts, the deadline state and the drafts.** A parent has no use for any of the three, and two of them are invisible to one by decision.
